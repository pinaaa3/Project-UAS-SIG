...
class PovertyMapController extends Controller
{
    public function index()
    {
        $regencies = Regency::with(['thematicData' => function ($query) {
            $query->select('regency_id', 'poverty_rate');
        }])
        ->get()
        ->map(function ($regency) {
            return [
                ...
                'name' => $regency->name,
                'thematic_data' => [
                    'value' => $regency->thematicData->first() ? $regency->thematicData->first()->poverty_rate : 0
                ]
            ];
        });
        ...
        return view('maps.poverty', compact('regencies', 'grades'));
    }

    public function getData()
    {
        $data = ThematicData::with('regency.province')
            ->select('regency_id', 'poverty_rate')
            ->get();
